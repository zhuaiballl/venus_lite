...
func LoadTipSet(ctx context.Context, tss string, api *porcelain.API) (*block.TipSet, error) {
	if tss[0] == '@' {
		if tss == "@head" {
		...
		return api.ChainGetTipSetByHeight(ctx, nil, abi.ChainEpoch(h), true)
	}
...
var stateSectorCmd = &cmds.Command{
	...
	Run: func(req *cmds.Request, re cmds.ResponseEmitter, env cmds.Environment) error {
		...
		ts, err := LoadTipSet(req.Context, req.Arguments[1], GetPorcelainAPI(env))
		if err != nil {
			return err
		}


		sectors, err := GetPorcelainAPI(env).StateMinerSectors(req.Context, maddr, nil, ts.Key())
		if err != nil {
			return err
		}
