...
func NewSyncerSubmodule(ctx context.Context,
	...
	tickets := consensus.NewTicketMachine(chn.ChainStore)
	...
	stmgr := statemanger.NewStateManger(chn.ChainStore, nodeConsensus, rnd,
...
func (syncer *SyncerSubmodule) handleIncomingBlocks(ctx context.Context, msg pubsub.Message) error {
	...
	_, err = syncer.ChainModule.ChainStore.PutObject(ctx, bm.Header)
	if err != nil {
...
func (syncer *SyncerSubmodule) loadLocalFullTipset(ctx context.Context, tsk types.TipSetKey) (*types.FullTipSet, error) {
	ts, err := syncer.ChainModule.ChainStore.GetTipSet(tsk)
	if err != nil {
		return nil, err
	}
